...
You can now remove the ${import_chalk.default.red("--preview-feature")} flag.`);
    ...
    const datasourceInfo = await (0, import_ensureDatabaseExists.getDatasourceInfo)({ schemaPath });
    (0, import_printDatasource.printDatasource)({ datasourceInfo });
    const migrate = new import_Migrate.Migrate(schemaPath);
    try {
      const wasDbCreated = await (0, import_ensureDatabaseExists.ensureDatabaseExists)("push", schemaPath);
      if (wasDbCreated) {
        console.info();
