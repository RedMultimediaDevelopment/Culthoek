...
var import_printDatasources = require("../utils/printDatasources");
var import_removeDatasource = require("../utils/removeDatasource");
const debug = (0, import_debug.default)("prisma:db:pull");
const _DbPull = class {
  static new() {
    return new _DbPull();
  }
  ...
  async parse(argv) {
    ...
    const spinnerFactory = (0, import_internals.createSpinner)(!args["--print"]);
    ...
    if (args["--clean"] || args["--experimental-reintrospection"]) {
      ...
      if (args["--experimental-reintrospection"]) {
        renamedMessages.push(
          `The ${import_chalk.default.redBright(
            "--experimental-reintrospection"
          )} flag has been removed and is now the default behavior of ${import_chalk.default.greenBright("prisma db pull")}.`
        );
      }
      if (args["--clean"]) {
        renamedMessages.push(
          `The ${import_chalk.default.redBright("--clean")} flag has been renamed to ${import_chalk.default.greenBright("--force")}.`
        );
      }
      console.error(`
...
`);
    ...
    if (schemaPath && !args["--print"]) {
      ...
      (0, import_printDatasource.printDatasource)({ datasourceInfo: await (0, import_ensureDatabaseExists.getDatasourceInfo)({ schemaPath }) });
    } else {
      (0, import_internals.loadEnvFile)(args["--schema"], false);
    }
    if (!url && !schemaPath) {
      throw new import_errors.NoSchemaFoundError();
    }
    const { firstDatasource, schema } = await (0, import_ts_pattern.match)({ url, schemaPath }).when(
...
${(0, import_removeDatasource.removeDatasource)(rawSchema)}`;
...
You can explicitly ignore and override your current local schema file with ${import_chalk.default.green(
...
Some information will be lost (relations, comments, mapped fields, @ignore...), follow ${(0, import_internals.link)(
    ...
    const engine = new import_MigrateEngine.MigrateEngine({
    ...
    try {
      ...
      if (e.code === "P4001" && introspectionSchema.trim() === "") {
        throw new Error(`
${import_chalk.default.red.bold(`${e.code} `)}${import_chalk.default.red("The introspected database was empty:")} ${url ? import_chalk.default.underline(url) : ""}

${import_chalk.default.bold("prisma db pull")} could not create any models in your ${import_chalk.default.bold(
          "schema.prisma"
        )} file and you will not be able to generate Prisma Client with the ${import_chalk.default.bold(
...
- make sure the database connection URL inside the ${import_chalk.default.bold("datasource")} block in ${import_chalk.default.bold(
          "schema.prisma"
        )} points to a database that is not empty (it must contain at least one table).

Then you can run ${import_chalk.default.green((0, import_internals.getCommandWithExecutor)("prisma db pull"))} again. 
`);
      } else if (e.code === "P1003") {
        throw new Error(`
${import_chalk.default.red.bold(`${e.code} `)}${import_chalk.default.red("The introspected database does not exist:")} ${url ? import_chalk.default.underline(url) : ""}
...
`);
        ...
        console.info();
        throw new Error(`${import_chalk.default.red(`${e.message}`)}
Introspection failed as your current Prisma schema file is invalid

Please fix your current schema manually (using either ${import_chalk.default.green(
...
Or run this command with the ${import_chalk.default.green(
          "--force"
...
${(0, import_internals.link)("https://pris.ly/d/upgrading-to-prisma2")}
` : "";
    if (args["--print"]) {
...
// introspectionSchemaVersion: ${introspectionSchemaVersion}`,
      ...
      const prisma1UpgradeMessageBox = prisma1UpgradeMessage ? "\n\n" + (0, import_internals.drawBox)({
        height: 16,
        width: 74,
        str: prisma1UpgradeMessage + "\nOnce you upgraded your database schema to Prisma 2+, \ncontinue with the instructions below.\n",
        horizontalPadding: 2
      }) : "";
      introspectionSpinner.success(`Introspected ${modelsAndTypesCountMessage} into ${import_chalk.default.underline(
        import_path.default.relative(process.cwd(), schemaPath)
      )} in ${import_chalk.default.bold((0, import_internals.formatms)(Date.now() - before))}${prisma1UpgradeMessageBox}
      ${import_chalk.default.keyword("orange")(introspectionWarningsMessage)}
${`Run ${import_chalk.default.green((0, import_internals.getCommandWithExecutor)("prisma generate"))} to generate Prisma Client.`}`);
    }
    return "";
  }
...
`;
  ...
  Field(s): "${fields.join('", "')}"`).join("\n");
        ...
        } else if (warning.code === 7 || warning.code === 14 || warning.code === 18 || warning.code === 19 || warning.code === 27 || warning.code === 30 || warning.code === 31) {
          message += warning.affected.map((it) => `- Model "${it.model}"`).join("\n");
        } else if (warning.code === 35) {
          ...
          message += warning.affected.map((it) => `- View "${it.view}", Field: "${it.field}", Type: "${it.tpe}"`).join("\n");
...
DbPull.help = (0, import_internals.format)(`
